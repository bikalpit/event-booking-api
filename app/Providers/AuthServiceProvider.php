...
class AuthServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $this->app['auth']->viaRequest('api', function ($request) {
            if ($request->header('api-token') && $request->header('admin-id') || $request->header('superadmin-id') || $request->header('event-org-id') || $request->header('order-mngt-id') || $request->header('overview-id')) {
                if(!empty($request->header('admin-id'))){
                  ...
                  $user_type = "EO";
                    ...
                    $user_type = "OM";
                  } elseif(!empty($request->header('overview-id'))) {
                    $user_id = $request->header('overview-id');
                    $user_type = "OV";
                  }
